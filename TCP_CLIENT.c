     ...
     while(1)
     {
    	 if (socket(0, Sn_MR_TCP, 3000, 0) == 0 ) {
    		...
    		if (connect(0,&destip,destport) == SOCK_OK) {
    			PRINT_STR("cnx is established\r\n");

    			 if(sockStatus = getSn_SR(0) == SOCK_ESTABLISHED) {

    				 PRINT_STR("Cnx established with remote peer\r\n");
    				 if(send(0, GREETING_MSG, strlen(GREETING_MSG)) == (int16_t)strlen(GREETING_MSG))
     					  PRINT_STR("Greeting msg was sent \r\n");
    			...
    			PRINT_STR("cnx was not established \r\n");
    		}
